...
class Mcqa(McqaInterface):
    ...
    def generate_query_response(self):
        ...
        """
        if (
            self.request.context.context_type == "pdf"
            or self.request.context.context_type == "text"
        ):
            ...
            if self.request.question_format == "raw":
                ...
                return self.postprocessor.postprocess(
                    generated_response=response, actual_answer=self.request.answer
                )
        ...
        if (
            ...
            if self.request.question_format == "synthetic":
                user_prompt, system_prompt = (
                    self.question_formulator.create_synthetic_questions(
                        question=self.request.query,
                        answer=self.request.answer,
                ...
                rephrased_questions = self.postprocessor.postprocess_synthetic(
